...
function setAdd(connection, setItems, items, item, algebra){
...
function indexOf(connection, props, items){
	var id = connection.id(props);

	for(var i = 0; i < items.length; i++) {
		var connId = connection.id(items[i]);
		if( id == connId) { //jshint ignore:line
			return i;
		}
	}
	return -1;
...
module.exports = function(options){
	...
	function makeBaseConnection(options) {
		var base = {		
		...
		Object.setPrototypeOf(base, options);
		return base;
	}


	Object.setPrototypeOf(behavior, makeBaseConnection(options || {}));
